...
def create_example_data():
    if not Cell.select():
        ...
        Cell.create(text="Pistol", name="Weapon", order=1, board=board1, status=Status.get(Status.status == "New"))
        Cell.create(text="Sword", name="Weapon", order=1, board=board1, status=Status.get(Status.status == "New"))
